...
<div class="container">
    <div class="row">
        <div class="col">
            ...
            <section class="my-4">
                <div class="row">
                    <div class="col"><h4>{{ __('New Module Workflow') }}</h4></div>
                </div>
            </section>

            <section class="my-4">
                <form action="{{ route('module-workflows.store') }}" method="post">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @if ($errors->has('module')) is-invalid @endif" name="module">
                            ...
                            @foreach ($modules as $module)
                                <option value="{{ $module->id }}" {{ $module->id == old('module') ? 'selected' : null }}>{{ $module->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @if ($errors->has('workflow')) is-invalid @endif" name="workflow">
                            ...
                            @foreach ($workflows as $workflow)
                                <option value="{{ $workflow->id }}" {{ $workflow->id == old('workflow') ? 'selected' : null }}>{{ $workflow->name }}</option>
                            @endforeach
                        </select>
